...
void create_file(char* filename, char* buffer)
{
    ...
    for (curr_idx = 0; *filename != '\0' && curr_idx < FILENAME_LENGTH - 1; curr_idx++, filename++)
        metadata->filename[curr_idx] = *filename;

    metadata->filename[curr_idx] = '\0';
